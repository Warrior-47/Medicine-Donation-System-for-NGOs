...
def dashboard(request):
    ...
    if ngo_name:
        # NGO search lives on search-service; the gateway routes /search/.
        return redirect(f'/search/ngo_search/{quote(ngo_name, safe="")}/')

    if request.user.is_authenticated:
        if request.user.is_ngo:
            context = NGO_MedicineListInfo.objects.filter(NGO_id=request.user.id)
        else:
            context = Donor_MedicineListInfo.objects.filter(Donor_id=request.user.id)
        ...
        if request.user.is_ngo:
            ...
            if form.is_valid():
                ...
                NGO_MedicineListInfo.objects.create(MedicineName=MedicineName,DosageAmount=DosageAmount,MedicinePriority=MedicinePriority
                                                ,AmountRequired=AmountRequired,NGO_id=request.user.id)
            ...
            if form.is_valid():
                ...
                Donor_MedicineListInfo.objects.create(MedicineName=MedicineName,DosageAmount=DosageAmount,PillsLeft=PillsLeft
                                                ,ExpiryDateImage=ExpiryDateImage,Donor_id=request.user.id)
